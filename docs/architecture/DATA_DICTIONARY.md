...
> **Verification status:** Verified against `drizzle/schema.ts` and the generated Drizzle artifacts through `0007_acoustic_black_crow` on 2026-08-19. The migration ledger is the application-history companion to this dictionary.
...
| `guidedRenderPackages` | Tenant/project | Immutable versioned provenance manifest derived only from a current locked Recipe and ready simplified Blueprint | Source IDs/versions, selected-role snapshot, renderer rule, explicit exclusions, approval/stale state and reason; no renderer call, image, provider credentials/task, quantity, inventory, price, charge, publication, checkout, delivery, or Lookbook data | `0007_acoustic_black_crow` |
| `guidedManualRenderHandoffs` | Tenant/project | One provider-neutral manual-handoff intent associated with an approved render package | Optional review note, requester/time, status and stale explanation; never a provider selection, transmission, task, credential, render result, asset, or commercial/publication record | `0007_acoustic_black_crow` |
