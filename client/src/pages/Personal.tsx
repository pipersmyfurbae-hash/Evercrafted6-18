    ...
    <PersonalLayout>
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        {isAllowed ? <>
          ...
          <section className="grid gap-5 lg:grid-cols-2">
            <Card className="border-white/10 bg-[#f6f0e5] shadow-none"><CardHeader><CardTitle>Private projects</CardTitle><CardDescription>{overview.data?.personalWorkspace.name ?? "Personal workspace"} is separate from every client workspace.</CardDescription></CardHeader><CardContent className="space-y-2">{overview.data?.privateProjects.map(project => <div key={project.id} className="rounded-xl border border-stone-200 p-3"><p className="text-sm font-medium">{project.name}</p><p className="mt-1 text-xs text-stone-500">Private workspace project</p></div>)}{overview.data?.privateProjects.length === 0 ? <p className="rounded-xl border border-dashed border-stone-300 p-3 text-sm text-stone-500">No private projects yet.</p> : null}</CardContent></Card>
            <Card className="border-white/10 bg-[#f6f0e5] shadow-none"><CardHeader><CardTitle>Integration readiness</CardTitle><CardDescription>Provider state is visible here without exposing credentials or adding external service calls.</CardDescription></CardHeader><CardContent className="space-y-2">{overview.data?.integrationReadiness.map(integration => <div key={integration.key} className="rounded-xl border border-stone-200 p-3"><div className="flex items-center justify-between gap-3"><p className="text-sm font-medium">{integration.label}</p><span className={`rounded-full px-2.5 py-1 text-xs ${integration.status === "ready" ? "bg-[#e4eee6] text-[#405c4c]" : "bg-stone-100 text-stone-600"}`}>{integration.status === "ready" ? "Ready" : "Unconfigured"}</span></div><p className="mt-1 text-xs leading-5 text-stone-500">{integration.detail}</p></div>)}</CardContent></Card>
          </section>
        </> : null}
      </div>
    </PersonalLayout>
