...
export const appRouter = router({
  ...
  guidedWreath: router({
    ...
    prepareRenderPackage: protectedProcedure
      .input(z.object({ workspaceId: z.number().int().positive(), projectId: z.number().int().positive() }))
      .mutation(async ({ ctx, input }) => {
        await requireWorkspaceRole({ userId: ctx.user.id, workspaceId: input.workspaceId, predicate: canOpenGuidedWreath });
        ...
        const project = await getGuidedProjectForWorkspace(input.projectId, input.workspaceId);
        if (!project) throw new TRPCError({ code: "NOT_FOUND", message: "Guided Wreath project not found in this workspace" });
        try { return await prepareGuidedRenderPackage({ ...input, createdByUserId: ctx.user.id }); }
        catch (error) { throw new TRPCError({ code: "BAD_REQUEST", message: error instanceof Error ? error.message : "Render package could not be prepared" }); }
      }),
    approveRenderPackage: protectedProcedure
      .input(z.object({ workspaceId: z.number().int().positive(), projectId: z.number().int().positive(), renderPackageId: z.number().int().positive() }))
      .mutation(async ({ ctx, input }) => {
        await requireWorkspaceRole({ userId: ctx.user.id, workspaceId: input.workspaceId, predicate: canOpenGuidedWreath });
        const project = await getGuidedProjectForWorkspace(input.projectId, input.workspaceId);
        if (!project) throw new TRPCError({ code: "NOT_FOUND", message: "Guided Wreath project not found in this workspace" });
    ...
    requestManualRenderHandoff: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const project = await getGuidedProjectForWorkspace(input.projectId, input.workspaceId);
        if (!project) throw new TRPCError({ code: "NOT_FOUND", message: "Guided Wreath project not found in this workspace" });
        try { return await requestManualRenderHandoff({ ...input, requestedByUserId: ctx.user.id }); }
        catch (error) { throw new TRPCError({ code: "BAD_REQUEST", message: error instanceof Error ? error.message : "Manual handoff could not be requested" }); }
      }),
  }),
  asset: router({
    list: protectedProcedure
